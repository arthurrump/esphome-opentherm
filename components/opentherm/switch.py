...
async def new_openthermswitch(config: Dict[str, Any], key: str, hub: cg.MockObj) -> cg.Pvariable:
    var = cg.new_Pvariable(config[CONF_ID], cg.RawExpression("&" + str(cg.safe_exp(getattr(hub, key)))))
